...
If an argument was given (high, medium, or low), call the `kit-set-energy` MCP tool with that level and confirm back.

If no argument was given, call the `kit-get-energy` MCP tool and report the result. If no level is set yet, prompt the user to set one.
